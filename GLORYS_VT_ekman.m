% GLORYS Volumetric Transport: In the Ekman Layer 
...
% volumetric transport between GLORYS and GLORYS
% load('date_num.mat')
...
%%
% curtailing v_600ri to only include the top 47m
v_ek = v_600ri(:,1:18,:);

% finding the indicies where the seasonal rotated v velocities are negative
% to diagnose onshelf transport
onshelf_idx = find(v_ek<0);
v_onshelf = zeros(size(v_ek));
v_onshelf(onshelf_idx) = v_ek(onshelf_idx);

% this is calculating the individual Volume transport at each location
% along the shelf at each of the depths from surface to ekm

for i = 1:204
onshelf_vx(:,:,i) = z_diff(:,1:18).*squeeze(v_onshelf(:,:,i)).*lon_diff';
end
...
for i = 1:204
    for j = 1:92
onshelf_ek_sea(j,i) = (sum(onshelf_vx(j,:,i),2,'omitnan'))/10^6;
    end
end

% finding the indicies where the seasonal rotated v velocities are positive
% to diagnose offshelf transport
offshelf_idx = find(v_ek>0);
v_offshelf = zeros(size(v_ek));
v_offshelf(offshelf_idx) = v_ek(offshelf_idx);

for i = 1:204
offshelf_vx(:,:,i) = z_diff(:,1:18).*squeeze(v_offshelf(:,:,i)).*lon_diff';
end
...
for i = 1:204
    for j = 1:92
offshelf_ek_sea(j,i) = (sum(offshelf_vx(j,:,i),2,'omitnan'))/10^6;
    end
end

% calculating the Volume transport in the top 47m
for i = 1:204
    for j = 1:92
...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_seasonal.jpg']);
...
set(gcf,'color','w');
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_seasonal_anom.jpg']);

%%
years = [{'2003','2004','2005','2006','2007','2008','2009','2010','2011','2012',...
    '2013','2014','2015','2016','2017','2018','2019'}];
offshelf_sea_sum = sum(abs(offshelf_ek_sea),1);
onshelf_sea_sum = sum(abs(onshelf_ek_sea),1);
...
set(gcf,'color','w');
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_budget_shelf.jpg']);
...
%reducing the depths to be the top 47m
v_74 = squeeze(v_4D(w_74i:e_74i,idxlatS,1:18,:));
...
for i = 1:204
onshelf_vx_74(:,:,i) = z_diff(:,1:18).*squeeze(v_74_on(:,:,i)).*dxS_m;
end

vx_ek_S(v_oni) = onshelf_vx_74(v_oni);
...
for i = 1:204
offshelf_vx_74(:,:,i) = z_diff(:,1:18).*squeeze(v_74_off(:,:,i)).*dxS_m;
end
vx_ek_S(v_offi) = offshelf_vx_74(v_offi);

% adding up all the depths to get the net vx at each gridcell across all
% months
for i = 1:204
    for j = 1:length(lon_74_span)
vx_74(j,i) = (sum(vx_ek_S(j,:,i),2,'omitnan'))/10^6;
    end
end
...
v_nord = squeeze(v_4D(w_nordi:e_nordi,idxlatN,1:18,:));
...
for i = 1:204
onshelf_vx_nord(:,:,i) = z_diff(:,1:18).*squeeze(v_nord_on(:,:,i)).*dxN_m;
end

vx_ek_N(v_oni) = onshelf_vx_nord(v_oni);
...
for i = 1:204
offshelf_vx_nord(:,:,i) = z_diff(:,1:18).*squeeze(v_nord_off(:,:,i)).*dxN_m;
end
vx_ek_N(v_offi) = offshelf_vx_nord(v_offi);

for i = 1:204
    for j = 1:length(lon_nord_span)
vx_nord(j,i) = (sum(vx_ek_N(j,:,i),2,'omitnan'))/10^6;
    end
end

%% Summing up the Volume Transport to Construct the Volume Budget

% summing up ekm onshelf and offshelf VX at Nord: 2D
for i = 1:204
    for j = 1:length(lon_nord_span)
...
% summing up ekm onshelf and offshelf VX at 74: 2D
for i = 1:204
    for j = 1:length(lon_74_span)
...
% summing up ekm onshelf and offshelf VX at Nord: 1D
for i = 1:204
    onshelf_N_sea(i) = sum(onshelf_N_2D(:,i),1,'omitnan');
    offshelf_N_sea(i) = sum(offshelf_N_2D(:,i),1,'omitnan');
end

% summing up ekm onshelf and offshelf VX at 74: 1D
for i = 1:204
...
xticklabels(years)
ylim([-3.25 2])
title('Volume Transport Budget of the 600 m Polygon (GLORYS)')
ylabel('Vol Tx [Sv]')

set(gcf,'color','w');
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_budget_ekpoly.jpg']);
...
set(gcf,'color','w');
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_budget_Nord.jpg']);
...
set(gcf,'color','w');
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_budget_74.jpg']);
...
% 
% mean_vx_ek = nanmean(net_sea_vx);
% mean_vx_N = nanmean(net_N_vx);
% mean_vx_S = nanmean(net_S_vx);
% sum_vx = mean_vx_ek+mean_vx_N-mean_vx_S;
% 
% figure
% hold on
...
% yline(mean_vx_ek,'Color',[0 0 0],'LineWidth',1);
% yline(mean_vx_N,'Color',[0.35 0.35 0.35],'LineWidth',1);
% yline(-mean_vx_S,'Color',[0.65 0.65 0.65],'LineWidth',1);
% 
% 
% legend('Outer Edge (mean: -1.1758 Sv)','Northern Boundary (mean: -0.0190 Sv)',...
%     'Southern Boundary (mean: 0.5180)','location','southoutside')
% % ylim([])
% xticks(1:12:204);
% xlim([1 204])
% xticklabels(years)
...
% set(gcf,'color','w');
% imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
% 'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_3net_polygon.jpg']);
...
set(gcf,'color','w');
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_3net_net_polygon.jpg']);
...
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_datapoints.jpg']);
...
title('Average Cross-Shelf Volume Transport in the top 47m: Nordostrundingen (GLORYS)')

% positive indicates offshelf and negative is onshelf
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_avg_nord.jpg']);
...
title('Average Cross-Shelf Volume Transport in the top 47m: 74^oN (GLORYS)')

% positive indicates offshelf and negative is onshelf
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_avg_74.jpg']);

%% eastern edge (600m contour)
...
title('Average Cross-Shelf Volume Transport in the top 47m: Eastern Edge (GLORYS)')

% positive indicates offshelf and negative is onshelf
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_avg_outer.jpg']);
...
% outer ekm isobath
jan_ek_vx = nanmean(vx(:,jan_i),2);
feb_ek_vx = nanmean(vx(:,feb_i),2);
mar_ek_vx = nanmean(vx(:,mar_i),2);
apr_ek_vx = nanmean(vx(:,apr_i),2);
may_ek_vx = nanmean(vx(:,may_i),2);
jun_ek_vx = nanmean(vx(:,jun_i),2);
jul_ek_vx = nanmean(vx(:,jul_i),2);
aug_ek_vx = nanmean(vx(:,aug_i),2);
sep_ek_vx = nanmean(vx(:,sep_i),2);
oct_ek_vx = nanmean(vx(:,oct_i),2);
nov_ek_vx = nanmean(vx(:,nov_i),2);
dec_ek_vx = nanmean(vx(:,dec_i),2);

vx_ek_monthly = cat(2,jan_ek_vx,feb_ek_vx,mar_ek_vx,apr_ek_vx,may_ek_vx,...
    jun_ek_vx,jul_ek_vx,aug_ek_vx,sep_ek_vx,oct_ek_vx,nov_ek_vx,dec_ek_vx);
...
figure
imagesc([1:12],dist_nord_km,1000*vx_ek_monthly)
set(gca,'Ydir','reverse');
clim([-50 50])
colormap(cmocean('balance','pivot',0))
...
subtitle('Top 47 m','FontSize',12)
set(gcf,'color','w');
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_heat_monthly.jpg']);
...
jan_vx_sum = nansum(jan_ek_vx);
feb_vx_sum = nansum(feb_ek_vx);
mar_vx_sum = nansum(mar_ek_vx);
apr_vx_sum = nansum(apr_ek_vx);
may_vx_sum = nansum(may_ek_vx);
jun_vx_sum = nansum(jun_ek_vx);
jul_vx_sum = nansum(jul_ek_vx);
aug_vx_sum = nansum(aug_ek_vx);
sep_vx_sum = nansum(sep_ek_vx);
oct_vx_sum = nansum(oct_ek_vx);
nov_vx_sum = nansum(nov_ek_vx);
dec_vx_sum = nansum(dec_ek_vx);
...
set(gcf,'color','w');
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_monthly_scatter.jpg']);
...
% m_grid('box','fancy','tickdir','out','fontsize',12,'fontname','helvetica',...     
%     'ytick',60:5:80,'xtick',-35:15:5);
% m_scatter(lon_600,lat_600,40,jun_ek_vx*1000,'Filled');
% clim([-15 15])
% cmocean('balance','pivot',0)
% h = colorbar;
% ylabel(h,'Volume Transport [mSv]')
% 
% title('Average June Cross-Shelf Volume Transport in the top 47m (GLORYS)')
% 
% imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
% 'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_ek_jun.jpg']);
...
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_yearly_lineplot.jpg']);
...
%%
figure
scatter(idx2,dist_nord_km,'k','Filled');
set(gca,'Ydir','reverse');

title('K-Means Clustering Results (2 clusters, GLORYS)')
xlabel('Cluster Number','FontSize',10)
ylabel('Distance from Nordostrundingen [km]','FontSize',10)

set(gcf,'color','w');
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/2clusters.jpg']);

% it looks like there's two prominant clusters here: the widening region
% and the widened region
...
u_3D = nanmean(u_4D,4);
u_ek_avg = nanmean(u_3D(:,:,1:18),3);

v_3D = nanmean(v_4D,4);
v_ek_avg = nanmean(v_3D(:,:,1:18),3);

speed_ek_avg = sqrt(u_ek_avg.^2+v_ek_avg.^2);
...
m_quiver(LON(1:2:end,1:10:end),LAT(1:2:end,1:10:end),u_ek_avg(1:10:end,1:2:end)',...
  v_ek_avg(1:10:end,1:2:end)',2.5,'k','LineWidth',0.5,'Clipping','on','AutoScale','on');
...
% ylabel(x,'Speed [m/s]')
title('Time Mean Flow and Cross-Shelf Volumetric Transport: GLORYS 2003-2019','FontSize',16)

set(gcf,'color','w');
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/flow_vx_heat.jpg']);
...
legend('Outer Edge (mean: -0.8013 Sv)','Northern Boundary (mean: 0.0163 Sv)',...
    'Southern Boundary (mean: 0.2711)','Residual (mean: -0.5139)','location','southoutside','Fontsize',12)
str = {'R_o_u_t_e_r_,_n_o_r_t_h_e_r_n = 0.2808','R_o_u_t_e_r_,_s_o_u_t_h_e_r_n = -0.8303',...
    'R_n_o_r_t_h_e_r_n_,_s_o_u_t_h_e_r_n = -0.3124','R_o_u_t_e_r_,_r_e_s_i_d_u_a_l = 0.9671',...
    'R_n_o_r_t_h_e_r_n_,_r_e_s_i_d_u_a_l = -0.3817','R_s_o_u_t_h_e_r_n_,_r_e_s_i_d_u_a_l = -0.6871'};
...
set(gcf,'color','w');
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/VX_Ekman/vx_3sides_resids.jpg']);
...
% cmocean('deep')
% clim([0 400]);
% m_contour(lonbath,latbath,bath',[100 300 500 600 700 1000],'linew',1.5,'color','w');
% 
% % m_quiver(LON(2:1:end,35:7:395),LAT(2:1:end,35:7:395),u_ek_avg(35:7:395,2:1:end)',...
% %     v_ek_avg(35:7:395,2:1:end)',2.5,'k','LineWidth',0.5,'Clipping','on');
% 
% m_gshhs_i('patch',[0.95 0.95 0.95],'edgecolor','k','linew',0.5);
% m_grid('box','fancy','tickdir','out','fontsize',12,'fontname','helvetica',...     
%     'ytick',60:5:80,'xtick',-35:15:5);
...
% set(gcf,'color','w');
% % imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
% % 'GLORYS/GLORYS Vertical Sections/flow_speed_corrected.jpg']);

%% Additional analysis on max onshore transport
...
lon_km_x = NaN(length(lon_600_poly)-1,1);

for jj = 1:(length(lon_600_poly)-1)
lon_km_x(jj,1) = haversine([lat_600_poly(jj),lon_600_poly(jj)],[lat_600_poly(jj+1),lon_600_poly(jj+1)]); 
% haversign calculates the geometric distance in km between two coordinates
end

% lon_km(end,1) = lon_km(end-1,1);
lon_m_x = lon_km_x*1000;
lon_m_x(end+1,1) = lon_m_x(end,1);

dist_nord_x = cumsum(lon_m_x);

dist_nord_km_x = dist_nord_x/1000;
xq = [0:100:900];
dist_nord_km_highres = interp1(dist_nord_km_x,dist_nord_km_x,xq);

for dd = 1:length(xq)
dist_100s_idx(dd) = find(abs(dist_nord_km_x-xq(dd))==nanmin(abs(dist_nord_km_x-xq(dd))));
end
...
imwrite(getframe(gcf).cdata, ['/Users/saravianco/Documents/Research/MATLAB/' ...
'GLORYS/GLORYS_VX_corrected/polygon_100kms.jpg']);